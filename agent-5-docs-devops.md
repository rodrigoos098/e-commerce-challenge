...
- **DTOs:** ...
- **Inertia.js + React:** ...
- **Frontend 100% Inertia (não consome a API REST diretamente):**
  O frontend utiliza Inertia.js para toda a comunicação com o server. Dados de página chegam via `Inertia::render()` (props), mutações via `router.post/put/delete`, e filtros/paginação via `router.get` com query params. Isso elimina duplicação de controllers, simplifica a autenticação (cookies nativos do Laravel, sem tokens), e oferece UX superior (sem flash de conteúdo vazio). A API REST (`/api/v1/...`) existe como interface independente para clientes externos (apps mobile, integrações de terceiros), é completamente testada e documentada via Swagger, e pode ser consumida por qualquer client HTTP. Ambas as camadas (Inertia e API) utilizam a mesma camada de Services/Repositories.
