            ...
            <a href={`/products/${product.slug}`} className="block overflow-hidden aspect-square bg-gray-100">
                ...
                {isOutOfStock && (
                    <div className="absolute inset-0 z-10 flex items-center justify-center bg-black/30 backdrop-blur-sm rounded-none">
                        <span className="rounded-full bg-white/90 px-4 py-1.5 text-xs font-bold text-gray-800">Esgotado</span>
                    </div>
                )}
                <img
            ...
            <div className="flex flex-1 flex-col p-4">
                ...
                {product.tags && product.tags.length > 0 && (
                    <div className="mb-2 flex flex-wrap gap-1">
                        {product.tags.slice(0, 2).map((tag) => (
                            <span key={tag.id} className="rounded-full bg-violet-50 px-2 py-0.5 text-xs text-violet-600 font-medium">
                                {tag.name}
                            </span>
                        ))}
                    </div>
                ...
                <div className="mt-auto pt-4 flex items-center justify-between gap-2">
                    ...
                    <button
                        ...
                        className="flex items-center gap-1.5 rounded-xl bg-violet-600 px-3 py-2 text-xs font-semibold text-white hover:bg-violet-700 active:scale-95 transition-all disabled:opacity-40 disabled:cursor-not-allowed shadow-sm"
                    >
