...
function SortIcon({ direction }: { direction?: 'asc' | 'desc' | null }) {
  ...
  return direction === 'asc' ? (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      className="h-3.5 w-3.5 text-kintsugi-500"
      viewBox="0 0 20 20"
      fill="currentColor"
    >
      <path
...
function buildPaginationHref(page: number, currentUrl: string, baseUrl?: string): string {
  ...
  return `${baseUrl ?? parsedUrl.pathname}${queryString ? `?${queryString}` : ''}`;
}
    ...
    <div className="bg-white rounded-xl border border-warm-200 shadow-xs overflow-hidden">
        ...
        <table className="w-full text-sm">
              ...
              {columns.map((col) => (
                ...
                >
                  {col.sortable && onSort ? (
                    ...
                    >
                      {col.label}
                      <SortIcon direction={sortKey === col.key ? (sortDir ?? null) : null} />
                    </button>
                  ) : (
          ...
          <tbody className="divide-y divide-warm-200">
            {loading ? (
              ...
              <tr>
                <td colSpan={columns.length} className="px-4 py-12 text-center text-warm-400">
                  {emptyMessage}
                </td>
              </tr>
            ) : (
      ...
      {pagination && pagination.last_page > 1 && (
        <nav
          ...
          className="flex items-center justify-between px-4 py-3 border-t border-warm-200 bg-warm-50"
        >
