...
const FEATURES = [
  {
    icon: (
      <svg
        ...
        className="h-9 w-9 text-kintsugi-500"
        fill="none"
        viewBox="0 0 24 24"
        stroke="currentColor"
        strokeWidth={1.5}
    ...
    icon: (
      ...
      >
        <path
          strokeLinecap="round"
          strokeLinejoin="round"
          d="M10.05 4.575a1.575 1.575 0 10-3.15 0v3m3.15-3v-1.5a1.575 1.575 0 013.15 0v1.5m-3.15 0l.075 5.925m3.075.75V4.575m0 0a1.575 1.575 0 013.15 0V15M6.9 7.575a1.575 1.575 0 10-3.15 0v8.175a6.075 6.075 0 006.075 6.075h2.1a6.075 6.075 0 006.075-6.075V4.575"
        />
      </svg>
    ),
  ...
  ceramicas: (
    ...
    >
      <path
        strokeLinecap="round"
        strokeLinejoin="round"
        d="M12 3C8 3 5 6 5 10c0 2.5 1 4.5 2.5 6H5l1 3h12l1-3h-2.5C18 14.5 19 12.5 19 10c0-4-3-7-7-7z"
      />
    </svg>
  ),
  texteis: (
...
export default function Home({ featured_products, categories, stats }: HomePageProps) {
  ...
  return (
    <PublicLayout
      title="Início"
      description="Presentes autorais, artesanato premium e design com curadoria para celebrar a beleza nas imperfeições."
    >
      {/* Hero */}
      ...
      {/* Categories — Promovido para primeira seção pelo @/arrange */}
      <section
        className="bg-warm-50 py-12 sm:py-16 organic-section-fade"
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <RevealSection>
            <div className="flex items-end justify-between mb-10">
              <div>
                <p className="text-xs font-semibold uppercase tracking-widest text-kintsugi-500 mb-1">
                  Explorar
                </p>
                <h2
                  id="categories-heading"
                  className="font-display text-2xl sm:text-3xl lg:text-4xl font-extrabold text-warm-700"
                >
                  Categorias
                </h2>
              </div>
              ...
              {featuredCategories.map((cat, idx) => (
                <RevealSection key={cat.id} delay={idx * 80}>
                  <Link
                    ...
                    className="group flex items-center gap-5 rounded-2xl border border-warm-200 bg-white p-6 sm:p-8 hover:border-kintsugi-200 hover:bg-kintsugi-50 transition-all duration-200 hover:-translate-y-0.5"
                  >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
              {otherCategories.map((cat, idx) => (
                <RevealSection key={cat.id} delay={(featuredCategories.length + idx) * 60}>
                  <Link
                    href={`/products?category_id=${cat.id}`}
                    className="group flex flex-col items-center gap-2 rounded-xl border border-warm-200 bg-white p-4 text-center hover:border-kintsugi-200 hover:bg-kintsugi-50 transition-all duration-200 hover:-translate-y-0.5"
                  >
      ...
      {/* ——— Kintsugi divider: categories → featured ——— */}
      <div className="mx-auto max-w-5xl px-8">
        <KintsugiDivider className="my-0" />
      </div>
      ...
      {/* Featured products */}
      <section
        className="bg-cream py-12 sm:py-16 organic-section-fade"
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <RevealSection>
            <div className="flex items-end justify-between mb-12">
              ...
              <Link
                href="/products"
                className="text-sm font-semibold text-kintsugi-500 hover:text-kintsugi-600 transition-colors"
              >
                Ver todos →
              </Link>
            </div>
      ...
      {/* Why us — Issue #3 /arrange: Compact grid instead of massive vertical gaps */}
      <section
        className="bg-warm-50 py-12 sm:py-16 organic-section-fade overflow-hidden"
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <RevealSection>
            <div className="max-w-3xl mb-12 sm:mb-16">
              <h2
                id="features-heading"
                className="font-display text-4xl sm:text-5xl lg:text-6xl font-extrabold text-warm-800 leading-tight tracking-tight"
              >
                A Essência do <span className="text-kintsugi-500">Feito à Mão</span>
              </h2>
              <p className="mt-6 text-xl text-warm-500 leading-relaxed">
                Não somos um marketplace convencional. Celebramos a resiliência, o tempo e a
                dedicação das mãos de quem cria.
              </p>
            </div>
          </RevealSection>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12 lg:gap-16">
            {FEATURES.map((feature, idx) => (
              <RevealSection key={feature.title} delay={idx * 100}>
                <div className="flex flex-col items-start gap-6 group">
                  <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-white border border-warm-200 shadow-sm group-hover:border-kintsugi-300 group-hover:-translate-y-1 transition-all duration-500">
                    <div className="text-kintsugi-500 transform group-hover:scale-110 transition-transform duration-500">
                      {feature.icon}
                    </div>
                  </div>

                  <div>
                    <h3 className="font-display text-2xl font-extrabold text-warm-800 mb-3 leading-tight group-hover:text-kintsugi-600 transition-colors duration-500">
                      {feature.title}
                    </h3>
                    <p className="text-lg text-warm-500 leading-relaxed">{feature.description}</p>
                  </div>
                </div>
              </RevealSection>
            ))}
      ...
      {/* CTA bottom */}
      <section className="bg-warm-800 py-12 sm:py-16">
        <RevealSection>
