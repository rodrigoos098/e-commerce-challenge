...
export default function TagsIndex({ tags }: TagsIndexProps) {
    ...
    function handleCreate(event: React.FormEvent<HTMLFormElement>) {
        ...
        router.post('/admin/tags', { name, slug: slug || null }, {
            onSuccess: () => {
                setName('');
                setSlug('');
            },
    ...
    function handleUpdate(event: React.FormEvent<HTMLFormElement>) {
        ...
        router.put(`/admin/tags/${editingTagId}`, { name: editName, slug: editSlug || null }, {
            onSuccess: () => {
        ...
        <AdminLayout title="Tags">
            <div className="space-y-6 p-6">
                <div className="flex flex-col gap-2">
                    ...
                    <p className="text-sm text-warm-500">Gerencie tags comerciais para merchandising e curadoria de produtos.</p>
                </div>
                ...
                <div className="grid gap-6 lg:grid-cols-[360px_minmax(0,1fr)]">
                    <div className="rounded-2xl border border-warm-200 bg-white p-5 shadow-xs">
                        ...
                        <form onSubmit={editingTagId ? handleUpdate : handleCreate} className="space-y-4">
                            <div>
                                <label htmlFor="tag-name" className="mb-1.5 block text-sm font-medium text-warm-600">Nome</label>
                                <input
                                    id="tag-name"
                                    value={editingTagId ? editName : name}
                                    onChange={(event) => editingTagId ? setEditName(event.target.value) : setName(event.target.value)}
                            ...
                            <div>
                                <label htmlFor="tag-slug" className="mb-1.5 block text-sm font-medium text-warm-600">Slug</label>
                                <input
                                    id="tag-slug"
                                    value={editingTagId ? editSlug : slug}
                                    onChange={(event) => editingTagId ? setEditSlug(event.target.value) : setSlug(event.target.value)}
                                    className="w-full rounded-xl border border-warm-200 bg-warm-50 px-4 py-2.5 text-sm focus:border-transparent focus:outline-none focus:ring-2 focus:ring-kintsugi-500"
                    ...
                    <div className="rounded-2xl border border-warm-200 bg-white p-5 shadow-xs">
                        <div className="mb-4 flex items-center justify-between">
                            <h2 className="text-sm font-semibold uppercase tracking-wider text-warm-500">Lista de tags</h2>
                                ...
                                <tbody className="divide-y divide-warm-100 bg-white text-sm text-warm-700">
                                    {tags.map((tag) => (
                                        <tr key={tag.id}>
                                            ...
                                            <td className="px-4 py-3">{tag.products_count}</td>
