                ...
                <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
                    <div>
                        ...
                        <div className="flex items-center gap-3">
                            <h1 className="text-2xl font-bold text-warm-700">Pedido #{String(order.id).padStart(5, '0')}</h1>
                            <StatusBadge status={order.status} />
                        </div>
                        <p className="mt-0.5 text-sm text-warm-500">Realizado em {formatDate(order.created_at)}</p>
                    </div>
                ...
                <div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
                            ...
                            <div className="divide-y divide-gray-100">
                                {order.items.map((item) => (
                                    <div key={item.id} className="flex items-center justify-between px-6 py-4">
                                        ...
                                        <span className="text-sm font-semibold text-warm-700">{formatCurrency(item.total_price)}</span>
                                    </div>
                                ))}
                            </div>
                    ...
                    <div className="space-y-6">
                        <div className="rounded-xl border border-warm-200 bg-white p-6 shadow-xs">
                            <h2 className="mb-4 text-sm font-semibold uppercase tracking-wider text-warm-600">Cliente</h2>
                            <div className="flex items-center gap-3">
                        ...
                        {shippingAddressLines.length > 0 && (
                            <div className="rounded-xl border border-warm-200 bg-white p-6 shadow-xs">
                                <h2 className="mb-4 text-sm font-semibold uppercase tracking-wider text-warm-600">Endereco de Entrega</h2>
                        ...
                        {billingAddressLines.length > 0 && (
                            <div className="rounded-xl border border-warm-200 bg-white p-6 shadow-xs">
                                <h2 className="mb-4 text-sm font-semibold uppercase tracking-wider text-warm-600">Endereco de Cobranca</h2>
                                <div className="space-y-0.5 text-sm text-warm-600">
                        ...
                        <div className="rounded-xl border border-warm-200 bg-white p-6 shadow-xs">
                            <h2 className="mb-4 text-sm font-semibold uppercase tracking-wider text-warm-600">Informacoes</h2>
                            <div className="space-y-2 text-sm">
                                <div className="flex justify-between">
                                    <span className="text-warm-500">Pedido no</span>
                                    <span className="font-mono text-warm-600">#{String(order.id).padStart(5, '0')}</span>
                                </div>
                                <div className="flex justify-between">
