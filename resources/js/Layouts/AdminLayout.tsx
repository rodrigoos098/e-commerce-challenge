...
const IconDashboard = () => (
  ...
  >
    <path
      ...
      d="M4 5a1 1 0 011-1h4a1 1 0 011 1v6a1 1 0 01-1 1H5a1 1 0 01-1-1V5zm10 0a1 1 0 011-1h4a1 1 0 011 1v2a1 1 0 01-1 1h-4a1 1 0 01-1-1V5zM4 15a1 1 0 011-1h4a1 1 0 011 1v4a1 1 0 01-1 1H5a1 1 0 01-1-1v-4zm10-2a1 1 0 011-1h4a1 1 0 011 1v6a1 1 0 01-1 1h-4a1 1 0 01-1-1v-6z"
    />
  </svg>
);
...
const IconTags = () => (
  ...
  >
    <path
      strokeLinecap="round"
      strokeLinejoin="round"
      d="M7 7h.01M7 3h5.586a2 2 0 011.414.586l5.414 5.414a2 2 0 010 2.828l-6.586 6.586a2 2 0 01-2.828 0L3.586 11.999A2 2 0 013 10.585V5a2 2 0 012-2h2z"
    />
  </svg>
);
...
function buildBreadcrumbs(
  ...
  const segments = path
    .replace(new RegExp(`^${appRoutes.admin.root}/?`), '')
    .split('/')
    .filter(Boolean);
  ...
  let accPath = appRoutes.admin.root;
...
const navItems: NavItem[] = [
  {
    ...
    icon: <IconDashboard />,
    matchPaths: [appRoutes.admin.dashboard],
  },
  {
    label: 'Produtos',
    href: appRoutes.admin.products.index,
    icon: <IconProducts />,
    matchPaths: [appRoutes.admin.products.index],
  },
  {
    label: 'Categorias',
    href: appRoutes.admin.categories.index,
    icon: <IconCategories />,
    matchPaths: [appRoutes.admin.categories.index],
  },
  {
    label: 'Tags',
    href: appRoutes.admin.tags.index,
    icon: <IconTags />,
    matchPaths: [appRoutes.admin.tags.index],
  },
  {
    label: 'Pedidos',
    href: appRoutes.admin.orders.index,
    icon: <IconOrders />,
    matchPaths: [appRoutes.admin.orders.index],
  },
  {
    label: 'Estoque Baixo',
    href: appRoutes.admin.stock.low,
    icon: <IconStock />,
    matchPaths: [appRoutes.admin.stock.root],
  },
];
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-2 px-6 py-5 border-b border-warm-800/60">
        <img src="/logo-white.svg" alt="Shopsugi Logo" className="h-9 sm:h-10 w-auto" />
      ...
      <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
        <p className="px-3 mb-2 text-xs font-semibold text-warm-500 uppercase tracking-wider">
          Menu
        </p>
        {navItems.map((item) => {
            ...
            {breadcrumbs.map((crumb, idx) => (
              <React.Fragment key={idx}>
                ...
                {crumb.href ? (
                  ...
                  <span className="text-warm-700 font-medium truncate">{crumb.label}</span>
                )}
              </React.Fragment>
            ))}
          </nav>
