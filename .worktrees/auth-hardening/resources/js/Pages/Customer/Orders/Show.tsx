        ...
        <PublicLayout title={`Pedido #${order.id}`}>
            <div className="mx-auto max-w-4xl px-4 py-10 sm:px-6 lg:px-8">
                <nav aria-label="Navegacao" className="mb-6 flex items-center gap-2 text-sm text-warm-400">
                    <Link href="/customer/orders" className="transition-colors hover:text-kintsugi-600">Meus Pedidos</Link>
                ...
                <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <h1 className="text-2xl font-extrabold text-warm-700 sm:text-3xl">Pedido #{order.id}</h1>
                        <p className="mt-1 text-sm text-warm-500">Realizado em {formatDate(order.created_at)}</p>
                    </div>
                    ...
                    <div className="space-y-4">
                        {order.items.map((item) => (
                            <div key={item.id} className="flex items-center gap-4">
                                ...
                                <p className="shrink-0 text-sm font-bold text-warm-700">{formatPrice(item.total_price)}</p>
                            </div>
                        ))}
                    </div>
