...
8. **Endereços no checkout:** O `Checkout.tsx` envia os campos de endereço como pares chave-valor separados (`shipping_name`, `shipping_street`, `shipping_city`, `shipping_state`, `shipping_zip`, `shipping_country` e os correspondentes `billing_*`). O wizard de 3 etapas (Entrega → Cobrança → Revisão) usa `react-hook-form` com `zodResolver` para validação client-side; o submit final chama `router.post('/customer/orders', data)`. Quando `same_billing = true`, os campos de cobrança são copiados automaticamente dos campos de entrega via `setValue`. O backend deve mapear esses campos diretamente para os modelos de endereço correspondentes.
